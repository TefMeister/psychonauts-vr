# Builds the real OpenVR per-eye geometry query POC (32-bit console EXE). Same LLVM-MinGW i686
# toolchain as every other tool in this project.
#
# Usage: powershell -File build.ps1
# Then run the resulting .exe directly - requires SteamVR + the null driver running (already
# confirmed enabled/persistent since notes/27).
...
$src = Join-Path $PSScriptRoot "ipd_query_poc.c"
$out = Join-Path $PSScriptRoot "ipd_query_poc.exe"
